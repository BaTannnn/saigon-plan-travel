...
public record SaveTripRequest(
    ...
    public SaveTripRequest {
        if (categorySlugs != null) {
            List<String> normalizedSlugs = new ArrayList<>(categorySlugs.size());

            for (String slug : categorySlugs) {
                normalizedSlugs.add(slug == null ? null : slug.trim());
            }

            categorySlugs = Collections.unmodifiableList(normalizedSlugs);
        }
    }
}
